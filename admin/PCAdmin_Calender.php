 	 ...
 	 <option value="" id="label_select_photographer" adr_trans="label_select_photographer">Select a Photographer</option>
	   ...
	   $photographers="select * from user_login where type_of_user='Photographer' and pc_admin_id='$_SESSION[admin_loggedin_id]' order by first_name";
        ...
        {?>
				  ...
				  <td align="left" style="color:#000080;padding-left:20px;">&nbsp; <?php if(@$_REQUEST['ph_name']) { echo strtoupper($_REQUEST['ph_name'])." <br>(Photographer's) Calendar."; } if(@$_REQUEST['notexist']==1) { echo "<span style='color:red;font-weight:400'>Please type and select the Photographer <br>&nbsp;&nbsp;from the dropdown suggestion list.</span>"; } ?> </td>
				  <td>
...
if(busyDate==1)calendar.gotoDate(busyDateIs);
	...
	{
	$appointments=mysqli_query($con,"select * from appointments where photographer_id!=0 and photographer_id in(select id from user_login where type_of_user='Photographer' and pc_admin_id='$_SESSION[admin_loggedin_id]') and date(from_datetime)=CURRENT_DATE order by from_datetime");
	}
